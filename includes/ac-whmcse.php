...
if ( ! class_exists( 'AC_WHMCSe' ) ) {
	class AC_WHMCSe {
		 ...
		 */
		private static function get_custom_admin_path() {

			if ( ! empty( $GLOBALS[ 'customadminpath' ] ) ) return $GLOBALS[ 'customadminpath' ];
		...
		public static function check_need_update( $url, $version, $module = 'whmcse' ) {
			...
			curl_setopt( $ch, CURLOPT_USERAGENT, 'tripflex/whmcs/' . $version );
			$release = curl_exec( $ch );
			curl_close( $ch );

			logModuleCall( $module, 'update check', $version, $release );

			if ( version_compare( $release, $version ) > 0 ) return true;
